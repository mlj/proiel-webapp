...
class SourcesController < ApplicationController
  ...
  def show
    ...
    @annotated_by_stats = sentences.annotated.where('annotated_by IS NOT NULL').group(:annotator).count.map { |k, v| [k.full_name, v] }
    @reviewed_by_stats = sentences.reviewed.where('reviewed_by IS NOT NULL').group(:reviewer).count.map { |k, v| [k.full_name, v] }
