  ...
  module Tagger
    class InstanceFrequencyMethod < TaggerAnalysisMethod
      ...
      def grab_data(form)
        ...
        result = Token.connection.select_all("SELECT morphtag, lemmata.lemma, lemmata.variant, count(*) AS frequency FROM tokens LEFT JOIN sentences ON sentence_id = sentences.id LEFT JOIN lemmata ON lemma_id = lemmata.id WHERE form = \"#{form}\" AND source_id IN (#{language.sources.map(&:id).join(',')}) AND reviewed_by IS NOT NULL GROUP BY morphtag, lemma_id", 'Token')
        result.collect! do |e|
