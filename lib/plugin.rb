...
module PROIEL
  ...
  def self.register_plugin(klass)
    ...
    begin
      ...
      when Exporter
        @@exporters << instance
      when MorphologyTagger
        @@morphology_taggers << instance
      else
  ...
  def self.load_plugins
    ...
    Rails.logger.info "Registered #{self.morphology_taggers.count} morphology taggers"
  end
  ...
  class MorphologyTagger < Plugin
    # True if this morphology tagger applies to the language provided.
    def applies?(language_tag)
      false
    end

    # Analyse a word form. Returns an array of potential analyses.
