...
class AlignedUnit < Hash
  ...
  def initialize(owordarray, twordarray, dict, identifier = nil, verbose = false, report_changes = true)
    ...
    @verbose = verbose
    @blacklist = []
    @duplications = owordarray.map(&:lemma_id) + twordarray.map(&:lemma_id)
    (owordarray.map(&:lemma_id).uniq + twordarray.map(&:lemma_id).uniq).each { |l| @duplications.delete_at(@duplications.index(l)) }
    @iscores = {}
  ...
  def save!
    Token.transaction do
      self.each do |tword, oword|
        raise "#{tword.form} is a Greek word" if tword.language.id == 4
        ...
        unless oword
          ...
          raise "#{oword.form} is not a Greek word" unless oword.language.id == 4
  ...
  def duplicated?(word)
    @duplications.include?(word.lemma_id)
  end
  ...
  def initial_alignment
    ...
    self.each_key do |tw|
      ...
      indices.each do |i|
        if @dict.rank(tw.lemma_id, @o_avail[i].lemma_id) == 1 and POS_DISTANCES[tw.lemma.part_of_speech.tag.first.downcase.to_sym][@o_avail[i].lemma.part_of_speech.tag.first.downcase.to_sym] < 45 and self.keys.select { |t| t != tw and t.lemma == tw.lemma}.size < 1
  ...
  def align(oword, tword)
    ...
    [oword.lemma_id, tword.lemma_id].each { |l| @duplications.delete(l) }
  end
...
# make greek a constant
  ...
  def invariable_score(oword, tword)
    ...
    points = POS_DISTANCES[tword.lemma.part_of_speech.tag.first.downcase.to_sym][oword.lemma.part_of_speech.tag.first.downcase.to_sym]
    log {STDERR.write("#{POS_DISTANCES[tword.lemma.part_of_speech.tag.first.downcase.to_sym][oword.lemma.part_of_speech.tag.first.downcase.to_sym]} points for POS...")}
