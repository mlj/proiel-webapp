...
if __FILE__ == $0
  ...
  # Make a dictionary based on a single chapter
  ORIG_SOURCE = 1
  sd = SourceDivision.find(621)
  ...
  if original_sd
    ...
    (ttokens_by_verse.keys & otokens_by_verse.keys).each do |v|
      d.update([[ttokens_by_verse[v].compact , otokens_by_verse[v].compact]])
    end
    d.make
    ...
    e.to_csv(8, lambda{|x| Lemma.find(x).export_form}, STDOUT)
    # And test the rankings function
    STDERR.write(e.equivalents(58774).map { |l| Lemma.find(l) })
  end
end
