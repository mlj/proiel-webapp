...
#FIXME          @template.content_tag(:p, ...)
          "<p>" + label_for(method, label_options) + "\n" + super + "</p>"
        end
      end_src
      class_eval src, __FILE__, __LINE__
    end
  
    def static_text_field(method, text = nil, options = {})
      label_options = label_options_from_options(options)
      label_for(method, label_options) + " <span>" + text + "</span>"
    end
